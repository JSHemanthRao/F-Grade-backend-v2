...
const spec = {
  ...
  info: {
    ...
    version: '1.0.0',
    description: [
      'Read-only Zoho CRM API for Microsoft Copilot Studio.',
      '',
      'CRITICAL DATA ACCURACY RULES:',
      '1. ALWAYS call the appropriate CRM tool before answering any CRM question involving a record count, date, user, module, or business metric. Never answer from model knowledge.',
      '2. Treat the tool response as the sole source of truth.',
      '3. When the user mentions a month, convert it to an exact half-open date range: June 2026 → from=2026-06-01, to=2026-07-01. July 2026 → from=2026-07-01, to=2026-08-01.',
      '4. The from date is inclusive (>=) and the to date is exclusive (<).',
      '5. For "leads created in June": module=Leads, date_field=Created_Time, from=2026-06-01, to=2026-07-01.',
      '6. For "leads modified in June": date_field=Modified_Time.',
      '7. For "deals closed in June": date_field=Closing_Date.',
      '8. If the tool returns zero records, say only: "No matching CRM records were found for the requested criteria." Do not add explanations about what dates the CRM contains.',
      '9. If the tool call fails, say: "I couldn\'t retrieve the CRM data because the CRM request failed." Never convert a failure into 0 records.',
      '10. Never add unsupported statements about what date ranges exist in the CRM unless the tool explicitly returned that evidence.',
      '11. Do not ask the user for technical parameters (page, per_page, fields, from, to, filter) when they can be inferred from the request.',
      '12. After the tool returns records, present them directly. Do not replace them with generic statements.',
    ].join(' '),
    license: {
      name: 'ISC',
    ...
    '/api/crm/count': {
      get: {
        ...
        description: [
          ...
          'Zero result: say "No matching CRM records were found for the requested criteria."',
          'Tool failure: say "I couldn\'t retrieve the CRM data because the CRM request failed."',
        ].join(' '),
        parameters: [
          {
            name: 'module',
          ...
          },
          {
            ...
            required: false,
            type: 'string',
            description: 'Inclusive start of the date range (>=). Convert month names to the 1st of the month: June 2026 → 2026-06-01. July 2026 → 2026-07-01. August 2026 → 2026-08-01.',
          },
          {
            name: 'to',
            in: 'query',
            required: false,
            type: 'string',
            description: 'Exclusive end of the date range (<). Convert month names to the 1st of the following month: June 2026 → 2026-07-01. July 2026 → 2026-08-01. August 2026 → 2026-09-01.',
          },
          {
            name: 'search',
            in: 'query',
    ...
    '/api/crm/query': {
      get: {
        ...
        description: [
          ...
          'After the tool returns, present the returned records directly. Do not substitute them with a generic statement.',
          'Zero result: say "No matching CRM records were found for the requested criteria."',
          'Tool failure: say "I couldn\'t retrieve the CRM data because the CRM request failed."',
          'Never claim all records are from a specific date unless the tool explicitly returned evidence for that.',
          'For list requests, use retrieval_mode=limited (explicit count) or retrieval_mode=filtered (date/filter). Never use all in normal Copilot conversations.',
        ].join(' '),
        parameters: [
          {
            name: 'module',
